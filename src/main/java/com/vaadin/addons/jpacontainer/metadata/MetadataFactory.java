...
import com.vaadin.addons.jpacontainer.metadata.PersistentPropertyMetadata.AccessType;
...
/**
 * Factory for creating and populating {@link ClassMetadata} and {@link EntityClassMetadata} instances.
 * 
...
public class MetadataFactory {
    ...
    public <T> ClassMetadata<T> getClassMetadata(Class<T> mappedClass,
        ...
        ClassMetadata<T> metadata = (ClassMetadata<T>) metadataMap.get(
                mappedClass);
        if (metadata != null) {
            return metadata;
        }
        ...
        if (entity != null) {
            ...
            metadata = new EntityClassMetadata<T>(
            ...
            if (!Modifier.isFinal(mod) && !Modifier.isStatic(mod) && !Modifier.
                ...
                if (isEmbedded(f)) {
                    ...
                    metadata.addProperties(
                            new PersistentPropertyMetadata(f.getName(), cm,
                            PersistentPropertyMetadata.PropertyKind.REFERENCE, f));
        ...
        for (Method m : type.getDeclaredMethods()) {
            ...
            if (m.getName().startsWith("get") && !Modifier.isStatic(mod) && m.
                ...
                try {
                    ...
                    setter = type.getDeclaredMethod("set" + m.getName().
