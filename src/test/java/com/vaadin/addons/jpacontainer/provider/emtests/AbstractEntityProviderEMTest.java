...
/**
 * Abstract test case for {@link EntityProvider} that should work
...
public abstract class AbstractEntityProviderEMTest {
    ...
    protected abstract EntityProvider<Person> createEntityProvider() throws Exception;
    ...
    protected void doTestGetEntityIdentifierAt(final List<Person> testData,
        ...
        for (int i = 0; i < testData.size(); i++) {
            assertEquals(testData.get(i).getId(), entityProvider.
                    getEntityIdentifierAt(filter,
                    sortBy, i));
        }
        assertNull(entityProvider.getEntityIdentifierAt(filter, sortBy, testData.
                size()));
    }
